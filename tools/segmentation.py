...
def random_color_masks(image):
  ...
  r[image==1], g[image==1], b[image==1] = colors[random.randrange(0, 1)]
  colored_mask = np.stack([r,g,b], axis=2)
  return colored_mask


def instance_segmentation(img_path, threshold=0.2, rect_th=3,
  ...
  indices = [index for index,cls in enumerate(pred_cls) if cls == "person"]

  mask = np.zeros_like(img)
  ind = 0
  for i in indices:
      if mask.sum() < masks[i].sum():
        mask = masks[i]
        ind = i
  mask = random_color_masks(mask).astype(np.float)
  mask[mask==0] = .7
  img = img * mask
